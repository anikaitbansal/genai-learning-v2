...
class Chatbot:
    def __init__(
        self, debug=False
    ):  # debug is a boolean parameter that allows us to enable or disable debug mode when creating an instance of the Chatbot class. When debug
        self.memory = MemoryManager(
            "cli-session"
        )  # This line creates an instance of the MemoryManager class and assigns
        self.debug = debug
    ...
    def run(self):
        while True:
            ...
            if (
                self.debug
            ):  # If debug mode is enabled, we print the user's input to the console for debugging purposes. This allows us to see exactly what the user has entered before we process it, which can be helpful for troubleshooting and understanding the flow of the program.
